...
page_fault (struct intr_frame *f)
{
  ...
  if(!not_present || !pagedir_get_page(thread_current()->pagedir, fault_addr))//!check_addr_safe(fault_addr,1,NULL)) //not present check : pt-write-code test
  {
    sys_exit(-112);
  }
  ...
  sptp =  upage_to_sp_table_pack(fault_addr);
